...
int main()
{
	...
	uint32_t len = 1024;
	uint16_t dataLen = 5 + 3 * NUM_LED;
	char test_seq[dataLen + 1];

	// Setup SPI
	bcm2835_spi_setBitOrder(BCM2835_SPI_BIT_ORDER_MSBFIRST);      // The default
	bcm2835_spi_setDataMode(BCM2835_SPI_MODE0);                   // The default
	bcm2835_spi_setClockDivider(BCM2835_SPI_CLOCK_DIVIDER_128);    // div by 32 = 7.8125MHz 
	...
	// {
		...
		// bcm2835_spi_transfern(h, strlen(h));
		// bcm2835_spi_transfern(test_seq, dataLen);
		// sleep(1);
		cout << "Send color 2..." << endl;
		genColorSeq(test_seq, dataLen, 0xAA, 0xCC, 0xEE); // color #AAAAAA
		bcm2835_spi_transfern(test_seq, dataLen);
		// sleep(1);
	// }
