...
def png2rgb(dimension=1,rotate=0):
    for filename in os.listdir('inputLEDPNG/'):
        if filename.endswith(".png"):
            print("Read file:" + filename)

            img = Image.open('inputLEDPNG/' + filename).convert('RGB')
            w, h = img.size
            ...
            output_path = 'outputRGB/' + os.path.splitext(filename)[0] + '.json'

            with open(output_path, 'w') as outfile:
                json.dump(imgArr.flatten().tolist(), outfile) # 1d array
            print("Save Json file(1d)(rotate",rotate*90 ,"):" + output_path)
