  ...
  updateSlider() {
    ...
    if (this.mgr.mode === "") {
      ...
      this.LEDsliders.map((LEDInput) => {
        const id = this.checkedDancerId;
        LEDInput.selector.value = this.mgr.control[id][this.mgr.timeInd[id]][
          "Status"
        ][LEDInput.slider.id]["name"];
      ...
      this.LEDsliders.map((LEDInput) => {
        const id = this.checkedDancerId;
        LEDInput.selector.value = this.mgr.newStatus[id][LEDInput.slider.id][
          "name"
        ];
        $(".selectpicker").selectpicker("refresh");
        LEDInput.slider.noUiSlider.set(
...
<<<<<<< HEAD
  ...
  addSlider(name, value = 0) {
    ...
    slider.noUiSlider.on("update", (value) => {
      ...
      if (this.mgr.mode !== "")
        this.mgr.updateControl(this.checkedDancerId, slider.id, Number(value));
    });
  ...
  addDownloadBtn() {
    ...
    document.getElementById("download-link").onclick = (e) =>
      this.mgr.download();
  }
