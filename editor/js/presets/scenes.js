...
class Scenes {
    ...
    _addTitle() {
        ...
        // Add click event
        addBtn.onclick = (e) => {
            const msg = "Do you want to save this Status to Scene ?\n(If yes, please Enter the name)";
    ...
    _createScene(name) {
        ...
        for (let i = 0; i < DANCER_NUM; ++i) {
            newScene["status"].push(this._mgr.control[i][this._mgr.timeInd[i]]["Status"]);
        }
    ...
    _delScene(id) {
        ...
        document.querySelector('#scenes-list').removeChild(document.getElementById(id));
        this._saveScenes();
    }
    _createTempScene() { // fired when ctrl + c
        console.log("_createTempScene", this._tmpScene);
        this._tmpScene = {};
        this._tmpScene["status"] = [];
        for (let i = 0; i < DANCER_NUM; ++i) {
            this._tmpScene["status"].push(this._mgr.control[i][this._mgr.timeInd[i]]["Status"]);
        }
    }
